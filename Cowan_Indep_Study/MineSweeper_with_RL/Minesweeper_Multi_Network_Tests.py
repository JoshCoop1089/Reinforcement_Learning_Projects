...
for axe, col in zip(ax[0], version):
    axe.set_title(col)

for axe, row in zip(ax[:,0], network_style):
    axe.set_ylabel(row, rotation=90, size='large')

for axe, col in zip(ax1[0], version):
    axe.set_title(col)

for axe, row in zip(ax1[:,0], network_style):
    axe.set_ylabel(row, rotation=90, size='large')
    
for axe, col in zip(ax2[0], version):
    axe.set_title(col)

for axe, row in zip(ax2[:,0], network_style):
    axe.set_ylabel(row, rotation=90, size='large')

for axe, col in zip(ax3[0], version):
    axe.set_title(col)

for axe, row in zip(ax3[:,0], network_style):
    axe.set_ylabel(row, rotation=90, size='large')
...
for i in range(3):
    for j in range(3):
        ...
        display_data = [avg_score_random, untrained_results, trained_results]
        ...
        # sns.violinplot(data = avg_score_random, color = 'b', legend = False, ax = ax1[i,j], inner = 'quartile')
        # sns.violinplot(data = untrained_results, color = 'r', legend = False, ax = ax1[i,j], inner = 'quartile')
        # sns.violinplot(data = trained_results, color = 'g', legend = False, ax = ax1[i,j], inner = 'quartile')
