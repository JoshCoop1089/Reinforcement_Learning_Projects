...
filters = [32,64]
kernel_size = [3,3]
dense_layer_nodes = [6*(dimension**2), dimension**2]
dropout_coef = 0.5
l2_val = 0.0001
...
# avg_score_untrained = [100*ms.optimal_play_percent(dimension, num_mines, score) for score in avg_clicks_untrained]
avg_score_trained = [100*ms.optimal_play_percent(dimension, num_mines, score) for score in avg_clicks_trained]


display_data = []
display_data.append(avg_score_random)
# display_data.append(avg_score_untrained)
display_data.append(avg_score_trained)
...
plt.title(f"Network Specifics -> Conv2D Filters: {filters} || Kernel Size: {kernel_size}" +
          ...
          f"\nRandom Avg Score: {mean_scores[0]}/100 || Trained Avg Score: {mean_scores[1]}/100")

plt.legend(labels = ['Random Choice', 'Trained'])
