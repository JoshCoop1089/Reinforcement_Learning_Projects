    ...
    while repeat:
        ...
        for most_freq_unknown in assumption_squares:
            ...
            if len(new_KM) != 0 or len(new_KS) != 0:
                ...
                # print(f"New Mines: {new_KM}")
                # print(f"New Safes: {new_KS}")
                for (x,y) in new_KM:
                    # print((x,y))
...
def advanced_location_selection(covered_board, reference_board, fact_dict):
    ...
    # V = [status, num_possible_mines, known_mines, hidden_neighbors, safe_neighbors]
    # Build percent chance of mine for neighboring unknowns from fact dict
    unknown_dict = {}
    ...
    # print("Lowest Percent Unknown Neighbor Counts:\n", possibles)
...
def run_advanced_agent(covered_board, reference_board, num_mines, globalInfo = False, advanced = False):
    """
    ...
    if not check_board(cbA, reference_board):
        print("BOARD INCORRECT HOW THE FK")
...
# Graphing changing board size for a single mine density
# Board Parameters
attempts = 20
mine_percent = 0.7
min_size = 3
...
dimensions = [x*2 for x in range(min_size,max_size)]
for dimension in dimensions:
    ...
    for index in range(attempts):
        ...
        total_score_adv2, random_guess_adv2 = run_advanced_agent(cover, rb, num_mines, False, False)
        adv2_end = time.time()
...
plt.title(f"{attempts} tries at solving {mine_percent*100}% Density Minesweeper using \n different agents with varying board sizes")
ax1.set_xlabel('Board Size')
ax1.set_ylabel('%Mines Safely Found')
ax1.plot(dimensions, mine_scores_basic, color = 'g', label = "Basic")
# ax1.plot(mine_percents, mine_scores_adv, color = 'r', label = "Advanced Sel Off")
ax1.plot(dimensions, mine_scores_adv2, color = 'b', label = "Advanced")
ax1.plot(dimensions, mine_scores_adv3, color = 'c', label = "Advanced with Sel")
ax1.legend()
...
ax2.plot(dimensions, random_moves_basic, color = "g", label = "Basic")
# ax2.plot(mine_percents, random_moves_adv, color = "r", label = "Advanced Sel Off")
ax2.plot(dimensions, random_moves_adv2, color = "b", label = "Advanced")
ax2.plot(dimensions, random_moves_adv3, color = "c", label = "Advanced with Sel")
ax2.legend()
...
ax3.plot(dimensions, times_basic, color = "g", label = "Basic")
# ax3.plot(mine_percents, times_adv, color = "r", label = "Advanced Sel Off")
ax3.plot(dimensions, times_adv2, color = "b", label = "Advanced")
ax3.plot(dimensions, times_adv3, color = "c", label = "Advanced with Sel")
ax3.legend()

plt.show()

# # Graphing Changing Mine Densities on a single board size
# # Board Parameters
# dimension = 10
# attempts = 100
# num_intervals = 20
