# -*- coding: utf-8 -*-
"""
Created on Thu Oct 28 20:13:57 2021
...
def generate_q_test_data(q_training_parameters, epsilon, q_network):
    dimension, num_mines, num_games_per_q_training, learning_param, min_delta = q_training_parameters
    state_counter = 1
    history = {}
    history_terminals = {}
    
    for game_num in range(num_games_per_q_training):
        if game_num in [num_games_per_q_training//4 -1, 2*num_games_per_q_training//4 -1 , 
                        3*num_games_per_q_training//4-1, num_games_per_q_training -1]:
            print(f"Starting Training Game {game_num + 1} out of {num_games_per_q_training}")
        game_over = False
...
def train_actor_critic_networks(q_training_parameters, actor_network, critic_network):
    ...
    for epsilon_counter in range(10,21):
        ...
        while not loss_plateau_reached:
            ...
            output = critic_network.fit(states, labels, callbacks = [best_model_checkpoint_callback])
            ...
            if fit_counter > 20:
                print("Checking for plateau")
                critic_network.load_weights(checkpoint_filepath)

                output = critic_network.fit(states, labels, epochs = 6, callbacks = [plateau_callback])
                full_loss.extend(output.history["loss"])
                full_accuracy.extend(output.history['loss'])
...
def run_actor_critic_update_after_every_epsilon_change(q_network_params, training_params, num_games_eval):
    ...
    plt.title('Actor-Critic Model Loss over Time\n Actor Updated Every Plateau\nDecreasing Epsilon from 50% to 0% by increments of 5%')
    plt.show()

    plt.plot(epochs, training_accuracy, 'bo')
    plt.xlabel('Number of Epochs')
    plt.title('Actor-Critic Model Accuracy over Time\n Actor Updated Every Plateau\nDecreasing Epsilon from 50% to 0% by increments of 5%')
    plt.show()
...
if __name__ == '__main__':
    ...
    run_actor_critic_update_after_every_epsilon_change(q_network_params, training_params, num_games_eval)
    #############################